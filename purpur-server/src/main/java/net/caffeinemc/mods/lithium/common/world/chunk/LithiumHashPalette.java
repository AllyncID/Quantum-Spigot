// Lithium by 2No2Name; LGPL-3.0. Port via DivineMC (NONPLAYT).
// Source: BX-Team/DivineMC@039981b0bf973ea9bee0f2521b2c01d9bb419802.
// Quantum adaptation: explicit opt-in integration; see docs/UPSTREAM_PATCHES.md.
...
public class LithiumHashPalette<T> extends HashMapPalette<T> implements Palette<T>, FastPalette<T> {
    ...
    private LithiumHashPalette(int indexBits, T[] entries, Reference2IntOpenHashMap<T> table, int size) {
        super(indexBits, true); // Quantum - preserve palette bit width when copying
